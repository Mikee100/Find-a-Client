      ...
      <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {item.status === "ACCEPTED" ? (
            <Link href={`/client/hire-requests/${item.id}`} className="rounded-md border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-700 hover:border-slate-900 hover:text-slate-900">
              Manage milestone
            </Link>
          ) : null}
