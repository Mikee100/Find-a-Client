      ...
      <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        {item.proposalMessage ? (
          <div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm">
            <p className="font-semibold text-blue-800">Developer proposal</p>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {item.threadId ? (
            <Link href={`/client/messages?thread=${encodeURIComponent(item.threadId)}`} className="rounded-md border border-slate-300 px-2.5 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50">
              Open thread
            </Link>
          ) : null}
          ...
          {item.status === "PROPOSAL_SENT" || item.status === "NEGOTIATING" ? (
            <>
              <button
                ...
                className="rounded-md border border-emerald-300 bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 hover:bg-emerald-100 disabled:opacity-60"
              >
                Accept
              </button>
              <button
          ...
          {(item.status === "PENDING" || item.status === "REVIEWING") ? (
            <button
              ...
              className="rounded-md border border-slate-300 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100 disabled:opacity-60"
            >
              Cancel request
            </button>
          ) : null}
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-6 md:px-6">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link href="/client/feed" className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">
            Back to feed
          </Link>
        </div>
