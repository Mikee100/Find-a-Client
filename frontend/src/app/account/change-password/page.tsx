...
import { changePassword, getAuthSession } from "@/lib/api";
...
export default function ChangePasswordPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const message = submitError instanceof Error ? submitError.message : "Unable to change password.";
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#F9FAFB] p-4">
      <section className="w-full max-w-md rounded-xl border border-[#E5E7EB] bg-white p-5 shadow-[0_8px_30px_rgba(0,0,0,0.08)]">
        ...
        <p className="mt-1 text-sm text-slate-600">Use your current password to set a new one.</p>
        ...
        <form onSubmit={onSubmit} className="mt-4 grid gap-2">
          <label htmlFor="currentPassword" className="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Current password</label>
          ...
          <p className={`text-xs ${strength.tone}`}>Password strength: {strength.label}</p>
        ...
        <div className="mt-4 flex items-center gap-3 text-sm text-slate-600">
          <Link href="/account/settings" className="font-semibold text-slate-900">Back to settings</Link>
          <span>-</span>
          <Link href="/login" className="font-semibold text-slate-900">Go to login</Link>
        </div>
      </section>
    </main>
  );
